...
@section('content')
    @php($canUpdate = can_access('ra_data_update'))
    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="row gy-4">
            @if(can_access('ra_data_review'))
                                            ...
                                            @foreach($data as $number => $ra_dates)
                                                @if($ra_dates)
                                                    ...
                                                    @else
                                                        <tr id="{{str_replace('hkg', '', $number)}}">
                                                            <td class="text-center">{{str_replace('hkg', '', $number)}}</td>
                                                            <td class="text-center">{{ ($ra_dates->next == '0000-00-00') ? '' : $ra_dates->next }}</td>
                                                            @for($i = 1 ; $i <=12; $i++)
                                                                <td class="text-center">{{ $ra_dates->{'last'.$i} }}</td>
                                                            @endfor
                                                        </tr>
                                                    @endif
                                                @endif
